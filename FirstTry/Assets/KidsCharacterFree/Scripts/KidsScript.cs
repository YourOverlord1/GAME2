...
public class KidsScript : MonoBehaviour
{
  ...
  //---------------------------------------------------------------------
  private const float Jump = 0.25f; // Изменено на float и добавлен 'f' для float-литерала
private const int Damage = 2;
private const int Faint = 3;
private Dictionary<int, bool> _Status = new Dictionary<int, bool>
{
    {(int)Jump, false }, // Приводим к int при добавлении в словарь
    {Damage, false },
    {Faint, false },
};
//------------------------------
private void STATUS()
{
    ...
    {
        _Status[(int)Jump] = true; // Приводим к int при обращении
    }
    else if(_Animator.GetCurrentAnimatorStateInfo(0).tagHash != JumpTag)
    {
        _Status[(int)Jump] = false;
    }
